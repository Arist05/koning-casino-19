...
return order;}
function buildGrid(){gridEl.innerHTML="";state.tiles=[];for(var i=0;i<TILE_COUNT;i+=1){var tile=document.createElement("button");tile.type="button";tile.className="tile-19a5a3fa";tile.disabled=true;tile.setAttribute("aria-label","Tile "+(i+1));tile.innerHTML='<span class="tile-inner-19a5a3fa">'+'<span class="tile-face-19a5a3fa tile-front-19a5a3fa"></span>'+'<span class="tile-face-19a5a3fa tile-back-19a5a3fa"></span>'+"</span>";tile.addEventListener("click",makeTileHandler(i));gridEl.appendChild(tile);state.tiles.push(tile);}}
function makeTileHandler(index){return function(){onTilePick(index);};}
function revealTile(index,isMine,dim){var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-19a5a3fa")){return;}
var back=tile.querySelector(".tile-back-19a5a3fa");if(back){back.innerHTML=isMine?BOMB_SVG:GEM_SVG;}
tile.classList.add("revealed-19a5a3fa");if(isMine){tile.classList.add("boom-19a5a3fa");}
if(dim){tile.classList.add("dimmed-19a5a3fa");}
tile.disabled=true;}
function revealRemaining(dimAll){for(var i=0;i<TILE_COUNT;i+=1){if(!state.tiles[i].classList.contains("revealed-19a5a3fa")){revealTile(i,state.mineMap[i]===true,dimAll);}}}
function setTilesEnabled(enabled){for(var i=0;i<TILE_COUNT;i+=1){var tile=state.tiles[i];tile.disabled=!enabled||tile.classList.contains("revealed-19a5a3fa");}}
...
var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-19a5a3fa")){return;}
...
function bust(){var lost=state.bet;revealRemaining(true);setStatus("Boom! The mine took your "+formatMoney(lost)+" bet.","lose");if(panelEl){panelEl.classList.remove("jolt-19a5a3fa");void panelEl.offsetWidth;panelEl.classList.add("jolt-19a5a3fa");}
endRound();}
